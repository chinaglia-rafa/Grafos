...
struct Grafo {
    ...
    char filename[200];
    short is_from_file;
    //  indica se o grafo foi editado e nao salvo
    short edited;
};
...
//  TODO: aprender a programar direito para nao precisar fazer isso
int dfs_tempo = 0;
...
short add_vertex(int vertex, struct Grafo* m) {
    ...
    char *log_text = (char*)malloc(sizeof(char) * 500);
    sprintf(log_text, "Criando vertice %d", vertex);
    log_to_file(log_text);
    free(log_text);
...
short add_edge(int o, int d, int p, struct Grafo* m) {
    ...
    if (m->item[o][d] != 0) {
        sprintf(log_text, "Aresta (%d, %d) ja existe com peso %d.\nAtualizando peso para %d.\n", indexAt(o, *m), indexAt(d, *m), m->item[o][d], p);
        log_to_file(log_text);
        free(log_text);
        printf("Aresta (%d, %d) ja existe com peso %d.\nAtualizando peso para %d.\n", indexAt(o, *m), indexAt(d, *m), m->item[o][d], p);
        m->item[o][d] = p;
        return 1;
    }
    m->item[o][d] = p;
...
short rm_vertex(int index, struct Grafo* m) {
    char *log_text = (char*)malloc(sizeof(char) * 500);
    sprintf(log_text, "Removendo vertice %d", indexAt(index, *m));
    log_to_file(log_text);
    free(log_text);
...
short adj_to_file(struct Grafo m, char *filename) {
    ...
    if (f == NULL) {
        printf("[ERRO]: Arquivo de salvamento de grafo nao encontrado!\n");
        return 0;
    }
    fprintf(f, "%d\n", m.type);
    fprintf(f, "%d\n", m.size);
...
struct Grafo load_grafo_from_file(char *filename) {
    ...
    log_to_file(log_text);
    if (DEBUG) printf("Numero de vertices: %d\n", size);
    sprintf(log_text, "Numero de vertices: %d", size);
    log_to_file(log_text);
    ...
    while (fscanf(f, "%d %d %d", &origin, &destiny, &peso) != EOF) {
        ...
        if (indexOrigin == -1) {
            if (DEBUG) printf("Novo vertice: %d\n", origin);
            sprintf(log_text, "Novo vertice: %d", origin);
            log_to_file(log_text);
        ...
        if (indexDestiny == -1) {
            if (DEBUG) printf("Novo vertice: %d\n", destiny);
            sprintf(log_text, "Novo vertice: %d", destiny);
    ...
    //  Verificando vertices sobrando
    if (DEBUG) printf("Agora que as arestas foram consideradas, precisamos ver se existem vertices isolados");
    ...
    for (int i = indexCounter; i < size; i++) {
        //  Comeca a criar novos indices seguindo do maior encontrado
        if (DEBUG) printf("\nCriando vertice %d sem arestas.", bigger + i - indexCounter + 1);
...
void tbl_prof_visit(struct Tabela_profundidade* t, int i, struct Grafo m) {
    ...
    if (DEBUG) printf("Visitando vertice %c (%d) no tempo %d\n", indexAt(i, m) + '0', i, dfs_tempo);
    for (int j = 0; j < m.size; j++) {
        if (m.item[i][j] == 0) continue;
...
int get_dijkstra_min(struct Tabela_dijkstra t) {
    int min = -1;
    //  Encontra o primeiro elemento na pilha para ser referencia de menor
...
/**
**  Monta o caminho minimo usando Bellman-Ford. Raiz = -1 indica que a raiz e o primeiro elemento.
**/
struct Tabela_bellford path_bellford(struct Grafo m, int raiz) {
    char *log_text = (char*)malloc(sizeof(char) * 500);
    sprintf(log_text, "Comecando caminho usando Bellman-Ford.");
    log_to_file(log_text);
    ...
    if (raiz == -1) index_raiz = 0;
        ...
        if (index_raiz == -1) {
            printf("[ERRO]: Raiz nao encontrada.");
    ...
    printf("Caminho minimo gerado usando Bellman-Ford!\n");
    sprintf(log_text, "Caminho minimo gerado usando Bellman-Ford!\n");
    log_to_file(log_text);
...
/**
**  Monta o caminho minimo usando Dijkstra. Raiz = -1 indica que a raiz e o primeiro elemento.
**/
struct Tabela_dijkstra path_dijkstra(struct Grafo m, int raiz) {
    char *log_text = (char*)malloc(sizeof(char) * 500);
    sprintf(log_text, "Comecando caminho usando Dijkstra.");
    log_to_file(log_text);

    for (int i = 0; i < m.size; i++) {
        for (int j = 0; j < m.size; j++) {
            if (m.item[i][j] < 0) {
                printf("[ERRO]: Aresta com peso negativo encontrada! Impossivel usar Dijkstra.");
    ...
    //  Contador responsavel por ver quantos elementos ainda estao na fila
    ...
    while (queue_count > 0) {
        ...
        if (DEBUG) printf("Menor distancia atual = %d (%d)\n", indexAt(current_vertex, m), tabela.linha[current_vertex].d);
        sprintf(log_text, "Menor distancia atual = %d (%d)\n", indexAt(current_vertex, m), tabela.linha[current_vertex].d);
        log_to_file(log_text);
    ...
    printf("Caminho minimo gerado usando Dijkstra!\n");
...
/**
**  Faz uma busca em largura. Raiz = -1 indica que a raiz e o primeiro elemento.
**/
struct Tabela_largura busca_largura(struct Grafo m, int raiz) {
    ...
    if (DEBUG) printf("Raiz do grafo e %d \n", index_raiz);
    while (current_index != -1) {
        if (DEBUG) printf("Buscando irmao branco de %d (indice %d)\n", indexAt(current_index, m), current_index);
        for (int i = 0; i < m.size; i++) {
            if (m.item[current_index][i] == 0) continue;
            if (DEBUG) printf("%d e adjacente a %d\n", indexAt(i, m), indexAt(current_index, m));
...
/**
**  Faz uma busca por profundidade. Raiz = -1 indica que a raiz e o primeiro elemento.
**/
...
void print_tabela_profundidade(struct Tabela_profundidade t) {
    printf("  Raiz e o elemento %d\n", indexAt(t.raiz, t.grafo));
    printf("   ___________________________\n");
    printf("  | vertice | cor |  d  |  f  |\n");
